...
class YandexMap extends StatefulWidget {
  ...
  /// Network tiles provider to show rest on map
...
class _YandexMapState extends State<YandexMap> {
  ...
  late final List<NetworkTileProvider> _tiles = widget.tiles;
  ...
  void _updateTiles() async {
    final tiles = widget.tiles.map((e) => e.toJson()).toList();

    final controller = await _controller.future;
    // ignore: unawaited_futures
    controller._updateTiles(tiles);
  }
  ...
  Map<String, dynamic> _creationParams() {
    ...
    final tiles = _tiles.map((e) => e.toJson()).toList();

    return {'mapOptions': mapOptions, 'mapObjects': mapObjects, 'tiles': tiles};
  }
}
